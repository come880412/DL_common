...
def fog(x, severity=1):
    ...
    max_val = x.max()
    if h < 512 and w < 512 :
        x += c[0] * plasma_fractal(mapsize = 512,wibbledecay=c[1])[:w, :h][..., np.newaxis]
    elif h < 1024 and w < 1024:
        x += c[0] * plasma_fractal(mapsize = 1024,wibbledecay=c[1])[:w, :h][..., np.newaxis]
    else:
        x += c[0] * plasma_fractal(mapsize = 2048,wibbledecay=c[1])[:w, :h][..., np.newaxis]
...
if __name__ in "__main__":
    ...
    parser.add_argument('--save_dir', type=str, required=True, help='Save image path')
    parser.add_argument('--severity', type=int, default=1, help='Noisy level (1 ~ 5)')
    args = parser.parse_args()

    print('Using create noisy data')

    noisy_methods = collections.OrderedDict()
    noisy_methods['Gaussian_Noise'] = gaussian_noise # 高斯雜訊
    noisy_methods['Shot_Noise'] = shot_noise         # 
    noisy_methods['Impulse_Noise'] = impulse_noise
    noisy_methods['Defocus_Blur'] = defocus_blur
    noisy_methods['Glass_Blur'] = glass_blur
    noisy_methods['Motion_Blur'] = motion_blur
    noisy_methods['Zoom_Blur'] = zoom_blur
    noisy_methods['Snow'] = snow
    noisy_methods['Frost'] = frost
    noisy_methods['Fog'] = fog
    noisy_methods['Brightness'] = brightness
    noisy_methods['Contrast'] = contrast
    noisy_methods['Elastic'] = elastic_transform
    noisy_methods['Pixelate'] = pixelate
    noisy_methods['JPEG'] = jpeg_compression
    noisy_methods['Speckle_Noise'] = speckle_noise # 斑紋雜訊
    noisy_methods['Gaussian_Blur'] = gaussian_blur
    noisy_methods['Spatter'] = spatter
    noisy_methods['Saturate'] = saturate # 飽和度

    image_path_list = glob(os.path.join(args.data_dir, "*.jpg")) + glob(os.path.join(args.data_dir, "*.png"))
    ...
    for idx, method_name in enumerate(noisy_methods.keys()):
        save_path = os.path.join(args.save_dir, method_name, f"severity_{args.severity}")
        os.makedirs(save_path, exist_ok=True)

        corruption = lambda clean_img: noisy_methods[method_name](clean_img, args.severity)

        tqdm_size = tqdm(image_path_list)
        for img_path in tqdm_size:
            img = Image.open(img_path).convert("RGB")
            noisy_img = np.uint8(corruption(convert_img(img)))
            noisy_img = cv2.cvtColor(noisy_img, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(save_path, img_path.split('/')[-1]), noisy_img)

            tqdm_size.set_description('Noisy means: [{}] Severity: [{}]'.format(method_name, args.severity))
        ...
        plt.subplot(5, 4, idx+2)
        plt.title(method_name)
        plt.xticks([])
        plt.yticks([])
        plt.imshow(cv2.cvtColor(noisy_img, cv2.COLOR_BGR2RGB))
    plt.savefig(os.path.join(args.save_dir, f"severity_{args.severity}.png"))
    plt.show()
